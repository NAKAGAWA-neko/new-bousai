...
// 大雨特別警報
const rainSplWarningChartConfig: ChartConfiguration<"bar", number[], string> = {
  type: "bar",
  data: {
    labels: [
      "2013",
...
let rainSpWarningChart: Chart | null = null;
...
rainSpWarningChart = generateChart(
  rainSplWarningChartConfig,
  "rainSpWarningChart"
);
...
window.addEventListener("resize", () => {
  if (rainSpWarningChart) {
    rainSpWarningChart.destroy();
    rainSpWarningChart = generateChart(
      rainSplWarningChartConfig,
      "rainSpWarningChart"
    );
  ...
  if (rainRecordStChart) {
    ...
    rainRecordStChart = generateChart(
      rainRecordStChartConfig,
      "rainRecordStChart"
    );
  }
});
